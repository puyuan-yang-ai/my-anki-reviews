...
`all-MiniLM-L6-v2` 实际上是 Sentence-BERT (SBERT) 框架下的一个具体模型实现。这种关系类似于：

```
Sentence-BERT (SBERT)  # 框架/方法论
    ├── all-MiniLM-L6-v2     # 具体实现模型之一
    ├── all-mpnet-base-v2    # 具体实现模型之一
    └── ...                  # 其他实现
```
...
所以说，虽然模型名字看起来与SBERT不同，但它们都是在SBERT框架下的具体实现。这些模型都可以通过`sentence-transformers`库来使用，都遵循SBERT的设计理念和使用方式。



您说得对，我为之前没有详细说明 Sentence-BERT (SBERT) 深表歉意。让我重新解释：
